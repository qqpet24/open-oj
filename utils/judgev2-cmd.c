//gcc judgev2-cmd.c -o judgev2-cmd -O2 -lm -std=c99 -lpthread -w
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
#include <sys/time.h>
...
int main(int argc,char* argv[]){
    int cpu = atoi(argv[1]);//ms
    int memory = atoi(argv[2]);//mb
    int max_file_size = atoi(argv[3]);//mb;
    char* test_in_path = argv[4];
    char* standard_out_path = argv[5];
    char* test_out_path = argv[6];
    char* test_error_path = argv[7];
    char* work_dir = argv[8];
    int language_code = atoi(argv[9]);
    char* judge_result_path = argv[10];
    ...
    if(pid == 0){
        ...
        // tmp_rlimit.rlim_cur = (cpu+1)*1000;
        // tmp_rlimit.rlim_max = (cpu+10)*1000;
        // setrlimit(RLIMIT_RTTIME, &tmp_rlimit);

        //mem b SIGSEGV
        //add 4mb free memory bonus because this program also needs memory
        int tmp_mem = (memory+4)<<20;
        tmp_rlimit.rlim_cur = tmp_mem;
        tmp_rlimit.rlim_max = tmp_mem+256<<20;
        ...
        switch(language_code){
            case 0:
                //JAVA, current not support
                execle("java","Main",(char*) NULL,envp);
                break;
            case 1:
                //CPP
        ...
        //parent
        pthread_t thread1;
        unsigned int max_run_time_us = (cpu+10)*1000;//us +10ms max_run_time
        struct kill_process_handler_args args;
        args.pid = pid;
        args.s = max_run_time_us;
        ...
        stdout=freopen(judge_result_path, "w", stdout);
        //printf("ms:%d mb:%d exitcode:%d exitsignal:%d flag:%s\n",total_runtime_ms,total_memory_mb,exitcode,exitsignal,FLAG);
        printf("ms:%d mb:%d exitcode:%d exitsignal:%d flag:%s\n",total_runtime_ms,total_memory_mb,exitcode,exitsignal,FLAG);
        remove(test_out_path);
        remove(test_error_path);
    }
}
