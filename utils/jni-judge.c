//gcc -I/usr/lib/jvm/java-1.17.0-openjdk-amd64/include/ -I/usr/lib/jvm/java-1.17.0-openjdk-amd64/include/linux/ -dynamiclib jni-judge.c -o jni-judge.so -std=c99 -lpthread -w -shared -fPIC
//请注意，这是个非常失败的设计，因为使用JNI时候不能调用fork()，否则会出现不可预料的后果
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
#include <sys/time.h>
...
JNIEXPORT jstring JNICALL Java_JudgeJNI_execute (JNIEnv *env, jobject obj, jint i1, jint i2 , jint i3, jstring str1, jstring str2, jstring str3, jstring str4){
    int cpu = (int) i1;//ms
    int memory = (int) i2;//mb
    int max_file_size = (int) i3;//mb;
    char* test_in_path = (*env)->GetStringUTFChars(env, str1, 0);
    char* standard_out_path = (*env)->GetStringUTFChars(env, str2, 0);
    char* test_out_path = (*env)->GetStringUTFChars(env, str3, 0);
    char* test_error_path = (*env)->GetStringUTFChars(env, str4, 0);
    ...
    if(pid == 0){
        ...
        sprintf(&buffer,"ms:%d mb:%d exitcode:%d exitsignal:%d flag:%s\n",total_runtime_ms,total_memory_mb,exitcode,exitsignal,FLAG);
        jstring result = (*env)->NewStringUTF(env,buffer); // C style string to Java String
        return result;
    }
}